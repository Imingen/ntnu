...
def a_star(start, end, board):
    ...
    while open_set:
        ...
        for nb in neighbors:
            ...
            tmp_g = current_node.g + 1 # Increas G by one, but just keep it in tmp var until more hecks
...
def main():
    # Define the board in a 2D array
    board_text = board_loader("board-1-4.txt")
   # redraw_board(board_text)
  ...
  #      for j, symbol in enumerate(line):
  #          label = Label(root, text = symbol)
   #         label.grid(row=i, column=)
   # root.mainloop()

    # Define the coordinates of start end end (goal state)
